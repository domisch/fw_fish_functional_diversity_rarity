...
phyloendemism <- function (x, phy, weighted=TRUE) {
  ...
  ### EDIT: load rangesizes of all species
  ### need to give the full range size of all species to all branches (and not the fraction of range size i the subset of the small number of species)
  fw_fish_rs <- read.csv(paste0(path, "/traits_eco/master_table_spp_fam_comm_ED_RS_volume_residuals.csv"), h=T)
  row.names(fw_fish_rs) <- fw_fish_rs$sequence_original
  # fw_fish_rs <- subset(fw_fish_rs, select=c(rs_sprior_m2)) # m2 species range size (rs_sprior * 1.000.000 (= number of grid cells where species occurrs)
  fw_fish_rs <- subset(fw_fish_rs, select=c(volume.x)) # average water volume in m3 per species (=at those grid cells where species occurs)
  fw_fish_rs <- as.data.frame(t(fw_fish_rs) )
  fw_fish_rs <- fw_fish_rs[ ,order(colnames(fw_fish_rs))]
  
  fw_fish_rs <- as.matrix (fw_fish_rs)
  ranges <- fw_fish_rs %*% phylomatrix
  ranges <- as.vector(ranges)
